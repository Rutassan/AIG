...
void main()
{
    ...
    float sunSource = clamp(sunIlluminance, 0.0, 1.0);
    float skySource = clamp(skyIlluminance, 0.0, 1.0);
    float retainedLocalLight = clamp(baseShade - sunVisibility * 0.78, 0.0, 1.0);
    float effectiveBaseShade = clamp(baseShade * (0.08 + skySource * 0.92) + retainedLocalLight * (1.0 - skySource) * 0.88, 0.0, 1.0);
    float effectiveSunVisibility = clamp(sunVisibility * (0.06 + sunSource * 0.94), 0.0, 1.0);
    ...
    float sourcePresence = clamp(max(effectiveBaseShade, max(effectiveSunVisibility, retainedLocalLight)), 0.0, 1.0);
    float directShadow = mix(1.0 - shadowStrength * (0.22 + sunSource * 0.78), 1.0, effectiveSunVisibility);
    float terrainShadow = mix(1.0 - shadowDepthStrength, 1.0, reliefAccent * 0.66 + effectiveSunVisibility * 0.34);
    float reliefLift = mix(0.88, 1.06, reliefAccent);
    float skyBounce = mix(0.64 + skySource * 0.12, 1.08 + ambientLiftStrength * 0.06, reliefAccent) * mix(0.46 + skySource * 0.22, 1.02 + ambientLiftStrength * 0.04, sourcePresence);
    float ambientLift = mix(0.56 + skySource * 0.14, 1.0 + ambientLiftStrength * 0.08, reliefAccent * 0.40 + sourcePresence * 0.60);
    float shadowPresence = clamp((1.0 - effectiveSunVisibility) * (0.58 + (1.0 - reliefAccent) * 0.28), 0.0, 1.0);
    float horizonDepth = clamp(horizonDepthStrength * (1.0 - effectiveSunVisibility) * (0.32 + (1.0 - reliefAccent) * 0.44), 0.0, 1.0);
    float reliefBridge = clamp((reliefAccent * 0.34 + effectiveSunVisibility * 0.16 + (1.0 - effectiveBaseShade) * 0.12) * reliefBridgeStrength, 0.0, 0.32);
    float shadowContour = clamp((1.0 - effectiveSunVisibility) * (0.34 + reliefAccent * 0.22 + (1.0 - effectiveBaseShade) * 0.20) * shadowContourStrength, 0.0, 0.34);
    float distanceShadowLift = clamp((1.0 - effectiveSunVisibility) * (0.18 + horizonDepth * 0.42 + (1.0 - reliefAccent) * 0.18) * distanceShadowLiftStrength, 0.0, 0.28);
    float distantSilhouette = clamp((1.0 - effectiveSunVisibility) * (0.12 + (1.0 - reliefAccent) * 0.22 + (1.0 - effectiveBaseShade) * 0.18) * distantSilhouetteStrength, 0.0, 0.24);
    float lightPlasticity = clamp((reliefAccent * 0.28 + effectiveSunVisibility * 0.16 + diffuse * 0.18 * sunSource + (1.0 - effectiveBaseShade) * 0.10) * lightPlasticityStrength, 0.0, 0.26);
    ...
    float lightMix = effectiveBaseShade
        * mix(1.0, mix(1.0, wrapDiffuse, sunSource), clamp(shaderStrength * 0.88, 0.0, 1.0))
        * mix(1.0, directShadow, clamp(shaderStrength, 0.0, 1.0))
        * terrainShadow
        * reliefLift
        * skyBounce
        * ambientLift;
    float ambientFloor = mix(0.004, 0.12, clamp(sourcePresence * 0.86 + reliefAccent * 0.14, 0.0, 1.0) * (0.18 + skySource * 0.82));
    ...
    float sunScatter = pow(max(dot(viewDir, sunDir), 0.0), 11.0) * (0.16 + effectiveSunVisibility * 0.26 + reliefAccent * 0.08) * warmLightStrength * (0.88 + sunScatterStrength * 0.55) * (0.08 + sunSource * 0.92);
    float foliageTranslucency = foliageMask * foliageTranslucencyStrength * max(dot(-viewDir, sunDir), 0.0) * (0.18 + effectiveSunVisibility * 0.22) * (0.12 + skySource * 0.88);
    float microOcclusion = clamp((1.0 - effectiveBaseShade) * (0.14 + (1.0 - effectiveSunVisibility) * 0.10 + stoneMask * 0.06 + woodMask * 0.04), 0.0, 0.20);
    ...
    vec3 shadowTint = mix(vec3(1.0), vec3(0.84, 0.91, 1.06), coolShadowStrength * (1.0 - effectiveSunVisibility) * (0.55 + stoneMask * 0.25 + leavesMask * 0.12));
    lit *= shadowTint;
    ...
    vec3 skyContourTint = mix(vec3(0.90, 0.96, 1.04), vec3(1.05, 1.00, 0.92), effectiveSunVisibility * 0.34 + reliefAccent * 0.16);
    float skyContour = clamp((horizonDepth * 0.38 + (1.0 - effectiveSunVisibility) * 0.10) * skyContourStrength, 0.0, 0.26);
    lit = mix(lit, lit * skyContourTint, skyContour);
    vec3 skyResponseTint = mix(vec3(0.92, 0.97, 1.03), vec3(1.05, 1.01, 0.95), effectiveSunVisibility * 0.42 + reliefAccent * 0.12);
    float skyResponse = clamp((0.04 + sourcePresence * 0.12 + effectiveSunVisibility * 0.10 + (1.0 - shadowPresence) * 0.04) * skyResponseStrength * (0.20 + skySource * 0.80), 0.0, 0.24);
    ...
    float atmosphereFog = clamp(fogFactor * (0.28 + shaderStrength * 0.42 + ambientLiftStrength * 0.08 + hazeStrength * 0.08) * (0.78 + skySource * 0.22) + heightFog * atmosphereStrength * (0.16 + ambientLiftStrength * 0.04 + hazeStrength * 0.03) * (0.68 + skySource * 0.32), 0.0, 1.0);
    ...
    float farReadability = clamp((fogFactor * 0.24 + distantSilhouette * 0.30 + horizonDepth * 0.18 + (1.0 - effectiveBaseShade) * 0.08) * farReadabilityStrength, 0.0, 0.24);
    ...
    vec3 secondaryBounceTint = mix(vec3(0.94, 0.98, 1.02), vec3(1.04, 1.00, 0.94), effectiveSunVisibility * 0.46 + reliefAccent * 0.18);
    float secondaryBounce = clamp((0.10 + effectiveSunVisibility * 0.18 + reliefAccent * 0.12 + foliageMask * 0.08) * secondaryBounceStrength * (0.22 + skySource * 0.78), 0.0, 0.42);
    ...
    lit = mix(lit, lit * mix(vec3(0.95, 0.99, 1.03), vec3(1.02, 1.00, 0.97), effectiveSunVisibility * 0.24 + heightFog * 0.30), farWorldCohesion);
    vec3 farGradientTint = mix(vec3(0.90, 0.95, 1.02), vec3(1.03, 0.98, 0.92), heightFog * 0.44 + effectiveSunVisibility * 0.18);
