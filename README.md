...
`AIG` — это воксельная игра на `C# + raylib`, вдохновлённая Minecraft.
...
- собственный рендер, который постепенно переходит от простого рисования кубов к текстурному `chunk mesh`-пайплайну.
...
- атмосферный туман;
- стилизованный свет;
- облегчённый shader-pass мира.

Дальнейшая цель — приблизить графику к аккуратному Minecraft-подобному виду, но сохранить нормальную производительность на обычном железе.
...
Цель проекта — не просто повторить Minecraft, а построить свою воксельную игру со своей архитектурой:
- со своим игровым кодом;
- со своей генерацией мира;
- со своим ботом;
- со своим рендером;
- с сильной автоматической тестовой базой.

## Лицензия

В репозитории есть файл [LICENSE](/home/rutasan/AIG/LICENSE).
